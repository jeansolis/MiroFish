            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but full section may not be complete) -->
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **校友_345（index=1）**: reason text...
        ...
        // Example: - 选择家长_601（index 0）: reason text...
        if (!headerMatch) {
        ...
        // Example: - **家长_601（index 0）**: reason text...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer exists, copy as default only if not placeholder text
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 (## xxx), as section title is shown in the outer layer
  ...
  // Fix non-contiguous ordered list numbering: when single-item <ol> tags are separated by paragraph content, maintain incrementing numbers
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown formatting, try removing thought process before returning
