...
const renderGraph = () => {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity toward center to cluster independent node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset scaled by distance ratio, ensuring curves are clearly visible
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if actual dragging started (exceeds threshold)
