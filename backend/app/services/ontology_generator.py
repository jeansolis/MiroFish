...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual. When a person does not belong to a more specific person type, they go here.
   - `Organization`: Fallback type for any organization. When an organization does not belong to a more specific organization type, it goes here.
...
- The text may mention various people, such as "elementary school teachers", "bystanders", "some netizen"
...
4. All entity types must be real-world agents that can speak publicly, not abstract concepts
