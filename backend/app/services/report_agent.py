...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generated (content only, does not mean entire section is complete)"""
        self.log(
...
This tool retrieves the complete panorama of simulation results, especially suitable for understanding event evolution. It will:
...
[Use Cases]
- Need different role perspectives on an event (How do students see it? Media? Officials?)
...
[Reminder] Number of report chapters: minimum 2, maximum 5. Content should be focused on core prediction findings."""

# ── Section generation prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
You are a "future prediction report" writing expert, currently writing one chapter of a report.
...
The simulation world is a rehearsal for the future. We injected specific conditions (simulation requirements) into the simulation world. The behavior and interactions of Agents in the simulation are predictions of future human behavior.
...
1. [Must call tools to observe the simulation world]
   ...
   - Each chapter must call tools at least 3 times (max 5) to observe the simulated world

2. [Must quote Agents' original words and actions]
   - Agent statements and behaviors are predictions of future human behavior
   - Use quote format to display these predictions, for example:
...
3. [Language consistency - quotes must match the report language]
   - Tool results may contain mixed-language expressions
   - When quoting tool results, translate them to match the report language fluently
...
- Do NOT use any Markdown headings (#, ##, ###, #### etc.) within chapters
- Do NOT add the chapter title at the beginning of content
- Chapter titles are added automatically by the system; you only write body text
- Use **bold**, paragraph breaks, quotes, and lists to organize content, but no headings

[Correct example]
```
This chapter analyzes the spread of public opinion. Through deep analysis of simulation data, we found...
...
═══════════════════════════════════════════════════════════════
[Available Search Tools] (call 3-5 times per chapter)
═══════════════════════════════════════════════════════════════

{tools_description}

[Tool usage tips - mix different tools, don't use just one]
- insight_forge: Deep insight analysis, automatically decomposes problems and retrieves facts/relationships from multiple dimensions
...
The system will execute the tool and return the result to you. You cannot write tool results yourself.

Option B - Output final content:
When you have gathered enough information through tools, output chapter content starting with "Final Answer:".
...
4. [Quote format - must be standalone paragraphs]
   ...
   > "The response pattern appeared rigid and slow in the fast-paced social media environment."
...
class ReportAgent:
    ...
    1. Planning phase: Analyze simulation requirements, plan report chapter structure
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        2. Bare JSON (response is entirely or a single line of tool call JSON)
        """
        tool_calls = []
        ...
        # Only try when format 1 does not match, to avoid false matches in body text
    ...
    def _is_valid_tool_call(self, data: dict) -> bool:
        """Validate if parsed JSON is a legitimate tool call"""
    ...
    def plan_outline(
        ...
        """
        logger.info("StartPlan report outline...")
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                logger.warning(f"Section {section.title} round {iteration + 1}  iteration: LLM return None")
            ...
            if has_tool_calls and has_final_answer:
                ...
                logger.warning(
                    ...
                    f"LLM output both tool call and Final Answer (round {conflict_retries}  conflicts)"
                )
            ...
            # ── Case 2: LLM rounded to call tools ──
            if has_tool_calls:
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM rounded to call {len(tool_calls)} tools, only executing first: {call['name']}")
            ...
            # Use this content directly as final answer, stop idle looping
            logger.info(f"Section {section.title} no 'Final Answer:' prefix detected, directly adopting LLM output as final content (tool calls: {tool_calls_count} times)")
        ...
        # Reached max iterations, force generating content
    ...
    def generate_report(
        ...
        Save to folder immediately after each section generation, no need to wait for entire report.
        File structure:
        reports/{report_id}/
        ...
        # If not provided report_id，Auto-generate
        if not report_id:
        ...
        try:
            ...
            ReportManager.update_progress(
                report_id, "planning", 5, "StartPlan report outline...",
                completed_sections=[]
            )
            ...
            if progress_callback:
                progress_callback("planning", 0, "StartPlan report outline...")
            ...
            ReportManager.update_progress(
                report_id, "planning", 15, f"Outline planning complete, {len(outline.sections)}Section",
                completed_sections=[]
            )
            ReportManager.save_report(report)
            ...
            if progress_callback:
                progress_callback("generating", 95, "Currently assembling complete report...")
            
            ReportManager.update_progress(
                report_id, "generating", 95, "Currently assembling complete report...",
                completed_sections=completed_section_titles
            )
    ...
    def chat(
        ...
        try:
            ...
            if report and report.markdown_content:
                # Limit report length，Avoid context being too long
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        different from agent_log.jsonl structured log.
        
        Args:
            report_id: Report ID
            from_line: Start reading from which line(For incremental retrieval，0 Means start from beginning)
        ...
        with open(log_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                total_lines = i + 1
                if i >= from_line:
                    # Preserve original log lines，Remove trailing newline
    ...
    def save_section(
        ...
        Called immediately after each section generation, enables section-by-section output
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove Markdown heading lines that duplicate the section title
        2. Convert all ### and lower headings to bold text
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if title duplicates section title (skip duplicates within first 5 lines)
                if i < 5:
                    if title_text == section_title or title_text.replace(' ', '') == section_title.replace(' ', ''):
                        skip_next_empty = True
                        continue
                
                # All heading levels (#, ##, ###, ####, etc.) convert to bold
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if it is a duplicate title(Same title appearing within 5 consecutive lines)
                is_duplicate = False
                ...
                if is_duplicate:
                    # Skip duplicate titles and blank lines after them
                    i += 1
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Other first-level titles to bold
                        processed_lines.append(f"**{title}**")
                    ...
                    if title in section_titles or title == outline.title:
                        ...
                        # Non-section second-level titles to bold
                        processed_lines.append(f"**{title}**")
                        processed_lines.append("")
                        prev_was_heading = False
                else:
                    # ### and belowlevel titles convert to bold text
                    processed_lines.append(f"**{title}**")
                    processed_lines.append("")
                    prev_was_heading = False
                ...
                # Skip separator lines immediately after titles
                i += 1
                continue
            
            elif stripped == '' and prev_was_heading:
                # Keep only one blank line after titles
        ...
        # Clean multiple consecutive blank lines(Keep at most 2)
        result_lines = []
        empty_count = 0
